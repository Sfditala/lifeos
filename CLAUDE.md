...
**الخطوة الحالية: Step 11 — الكود جاهز، بانتظار مراجعة المستخدمة.**
**الحالة: Step 10 أُغلق (تسجيل الدخول عبر Magic Link تحقّقت منه المستخدمة فعليًا ونجح). Step 11 نُفّذ كودياً: تبويبات موحّدة لصفحة مجال الحياة (نظرة عامة/مشاريع/مهام/ملاحظات/اجتماعات/ملفات) + إضافة متفق عليها بالنقاش: صفحة مشروع مستقلة (وصف + تاريخ استحقاق + معالم رئيسية + مهامه الخاصة). تبويبا اجتماعات وملفات فاضيان مؤقتًا كما هو مخطط (Step 12/13). البناء نظيف، RLS متحقق منه على `project_milestones`. بانتظار مراجعة المستخدمة قبل الانتقال لأي خطوة تالية — القرار المتفق عليه هو Step 14 (تعديل/حذف حقيقي) بعدها.**
...
- [ ] **Step 11** — قالب صفحة مجال الحياة الغني (تبويبات موحّدة لكل مجال) ← **الخطوة الحالية**، + صفحة مشروع مستقلة (إضافة متفق عليها)
