...
**الخطوة الحالية: Step 14 — الكود جاهز (+تحسين على Step 11)، بانتظار مراجعة المستخدمة.**
**تحسين إضافي بعد ملاحظة المستخدمة:** تبويبا "اجتماعات" و"ملفات" لم يعودا يظهران دائمًا لكل مجال — كل مجال حياة فيه خيار اختياري (checkbox بديالوج الإضافة/التعديل) يحدد هل يحتاج هذا التبويب أو لا. مفعّلين حاليًا فقط على "الوظيفة" و"الشركة" (تبويب اجتماعات) بناءً على طلب المستخدمة المباشر.
**الحالة: Step 11 أُغلق. Step 14 نُفّذ كودياً (تعديل + حذف + سلة محذوفات، بالاتفاق مع المستخدمة تُبنى مع بعض): عمود `deleted_at` على الجداول الثمانية، تعديل وحذف حقيقي لكل كيان (مجالات حياة، أهداف، مشاريع، مهام، عادات، محتوى، ملاحظات، معالم رئيسية) عبر قائمة "⋮" بكل صف، تحذير عدّاد حقيقي عند حذف مجال/مشروع فيه كيانات تابعة (تُنقَل معه لسلة المحذوفات)، صفحة `/trash` للاسترجاع أو الحذف النهائي، وVercel Cron يومي يمسح نهائيًا أي شيء بسلة المحذوفات أقدم من 30 يوم. تحقّقت من آلية الحذف الناعم فعليًا (إنشاء→حذف→تحقق من الاختفاء→استرجاع→تنظيف). البناء نظيف. بانتظار مراجعة المستخدمة قبل الانتقال لأي خطوة تالية.**
...
- [ ] **Step 14** — CRUD كامل فعليًا (تعديل + حذف حقيقي + سلة محذوفات) ← **الخطوة الحالية**
